      ...
      try {
        mkdirSync(outputPath, () => {
          ...
          for (key in glyphs) {
            ...
            var glyphName =
              glyphs[key].options.name ||
              glyphs[key].options.glyphName ||
              key.slice(1);
            svgs.push(
              [
                `<li class="dib">`,
                `  <img class="icon svg-icon" src="data:image/svg+xml;base64,${new Buffer(
                  svg
                ).toString("base64")}" alt=""></img>`,
          ...
          let template = fs
            .readFileSync(path.join(__dirname, "template", "demo_svg.html"))
            .toString();
      ...
      try {
        ...
        var readSvg = function (dir) {
          ...
          demoUnicodeList.push(
            [
              `<li class="dib">`,
              `  <span class="icon ${fontFamily}">&#x${unicodeNum.toString(
                16
              )};</span>`,
              `  <div class="name">${glyphName}</div>`,
              `  <div class="code-name">&amp;#x${unicodeNum.toString(
                16
              )};</div>`,
              `</li>`
            ].join("\n")
          ...
          symbols.push(
            `<symbol id="${className}" viewBox="0 0 1024 1024">${pathNode}</symbol>`
          );
          const items = [
            `.${className}:before {`,
            `  content: "${content}";`,
        ...
        var outputCssJs = function () {
          //Output Css File
          const woff2 = fs
            .readFileSync(path.join(outputPath, fileName) + ".woff2")
            .toString();
        ...
        var outputDemo = function () {
          //Export demo-html
          let demoHtmlStr = fs
            .readFileSync(path.join(__dirname, "template", "demo_index.html"))
            .toString();
          demoHtmlStr = demoHtmlStr.replace(
            '<div id="unicode-list"></div>',
            demoUnicodeList.join("\n\n")
          );
          demoHtmlStr = demoHtmlStr.replace(
          ...
          fs.writeFileSync(
            path.join(outputPath, "demo_index.html"),
            demoHtmlStr
          );

          //Export demo-css
          fs.copyFileSync(
            path.join(__dirname, "template", "demo.css"),
            path.join(outputPath, "demo.css")
          );
        };
